...
export default function CompanyProfile() {
  ...
  return (
    <div className="pb-32">
      ...
      {/* Sticky CTA Bar */}
      <div className="fixed bottom-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-xl border-t border-slate-200 shadow-[0_-4px_24px_rgba(15,23,42,0.06)]" data-testid="sticky-cta">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex items-center justify-between gap-3">
          <div className="flex items-center gap-3 min-w-0">
            {c.logo_url ? <img src={c.logo_url} alt="" className="w-9 h-9 rounded-lg object-cover" /> : <div className="w-9 h-9 rounded-lg bg-blue-100 text-blue-700 font-bold flex items-center justify-center">{c.name?.[0]}</div>}
            <div className="min-w-0 hidden sm:block">
              <div className="font-semibold text-slate-900 text-sm truncate">{c.name}</div>
              <div className="text-xs text-slate-500 flex items-center gap-1"><Star className="w-3 h-3 fill-amber-400 text-amber-400" />{c.rating?.toFixed(1)} • {c.review_count} rəy</div>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <Button variant="outline" size="sm" className="hidden sm:flex" data-testid="sticky-shortlist" onClick={onShortlist}><Heart className="w-3.5 h-3.5 mr-1" />Shortlist</Button>
            <Button variant="outline" size="sm" data-testid="sticky-message" onClick={requireAuth(() => navigate("/buyer/messages"))}><MessageSquare className="w-3.5 h-3.5 mr-1" />Mesaj</Button>
            <Button size="sm" className="bg-blue-600 hover:bg-blue-700" data-testid="sticky-brief" onClick={requireAuth(() => navigate("/buyer/briefs/new", { state: { providerId: c.id } }))}><FileText className="w-3.5 h-3.5 mr-1" />Brief göndər</Button>
          </div>
        </div>
      </div>
    </div>
