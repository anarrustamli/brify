...
export default function ServiceDetail({ buyerMode = false }) {
  const { id } = useParams();
  ...
  const sourceReturnTo = location.state?.returnTo || "";
  ...
  const companyPath = `${buyerMode ? "/buyer/company" : "/companies"}/${s.company?.slug}`;
  const searchPath = buyerMode ? "/buyer/search/services" : "/services";
  const backTo = sourceReturnTo || searchPath;
  const openBrief = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <nav className="text-sm text-slate-500 mb-4">
        <Link to={backTo} className="hover:text-blue-600">Xidmətlər</Link> / <span className="text-slate-900">{s.name}</span>
      </nav>
      <div className="grid lg:grid-cols-[1fr_320px] gap-8">
        <div>
          <div className="bg-white border border-slate-200 rounded-2xl p-8">
            <Link to={companyPath} state={{ returnTo: backTo }} className="flex items-center gap-2 mb-4">
        ...
        <aside className="space-y-4">
          <div className="bg-white border border-slate-200 rounded-xl p-5 sticky top-20">
            ...
            <Link to={companyPath} state={{ returnTo: backTo }} className="flex items-center gap-3 mt-3 group">
            ...
            <Button className="w-full mt-5 bg-blue-600 hover:bg-blue-700" data-testid="brief-btn" onClick={openBrief}>
              Brief göndər
            </Button>
            <Button variant="outline" asChild className="w-full mt-2">
              <Link to={companyPath} state={{ returnTo: backTo }}>Profili gör</Link>
            </Button>
          </div>
        </aside>
      </div>
      <BriefSendDialog
        company={s.company}
        context={{ companyId: s.company_id, providerId: s.company_id, serviceId: s.id, sourcePage: "service", returnTo: sourceReturnTo || currentPath }}
        open={briefOpen}
