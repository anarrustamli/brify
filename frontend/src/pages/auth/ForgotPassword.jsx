...
export default function ForgotPassword() {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      setSent(true);
      toast.success("Bərpa linki göndərildi");
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-7">
          ...
          {sent ? (
            <div className="mt-6 rounded-xl border border-emerald-100 bg-emerald-50 p-4 text-sm text-emerald-800">
              Əgər email sistemdə varsa, bərpa linki göndərildi.
            </div>
          ) : (
            <form onSubmit={submit} className="mt-6 space-y-3">
              ...
              <Button type="submit" disabled={loading} className="w-full h-11 bg-blue-600 hover:bg-blue-700" data-testid="forgot-submit">{loading ? "Göndərilir..." : "Linki göndər"}</Button>
            </form>
          )}
