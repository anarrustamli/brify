            ...
            <div key={l.id} className="bg-white border border-slate-200 rounded-xl p-5">
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1"><StatusBadge status={l.status} /><span className="text-xs text-slate-500">{timeAgo(l.created_at)}</span>
                  {l.expires_at && new Date(l.expires_at) > new Date() && (
                    <span className="text-xs text-amber-700 bg-amber-50 px-2 py-0.5 rounded-full">
                      {Math.ceil((new Date(l.expires_at) - new Date()) / (24 * 3600 * 1000))} gün qalıb
                    </span>
                  )}
                  </div>
                  {l.status === "locked" ? (
                    <div className="mt-2 bg-amber-50 border border-amber-200 rounded-lg p-3" data-testid={`locked-lead-${l.id}`}>
                      <div className="text-sm font-semibold text-amber-900">Lead aylıq limit səbəbi ilə kilidlənib</div>
                      <div className="text-xs text-amber-800 mt-1">Buyer və brief detalları gizlədilib. Plan yüksəltməklə açın.</div>
                      <Button asChild size="sm" className="mt-2 bg-blue-600 hover:bg-blue-700" data-testid={`btn-upgrade-${l.id}`}>
                        <a href="/provider/billing">Planı yüksəlt →</a>
                      </Button>
                    </div>
                  ) : l.status === "expired" ? (
                    <div className="mt-2 bg-slate-50 border border-slate-200 rounded-lg p-3 text-sm text-slate-600" data-testid={`expired-lead-${l.id}`}>
                      Bu lead-in vaxtı bitib. Yeni dəvətlər üçün gözləyin.
                    </div>
                  ) : (
                  <>
                ...
                {l.status !== "locked" && l.status !== "expired" && l.status !== "proposal_sent" && l.status !== "accepted" && l.status !== "rejected" && (
